...
#include "../util/runtime/allocate.h"  // Use 'kissat' allocator if embedded.
#include "../util/runtime/error.h"     // Use 'kissat_fatal' if embedded.
#include "../core/main/internal.h"  // Also use 'kissat' statistics if embedded.
#include "../util/runtime/terminate.h" // For macros defining termination macro.
...
static unsigned import_literal (kitten *kitten, unsigned elit) {
  ...
  if (!iidx) {
    iidx = SIZE_STACK (kitten->exports);
    PUSH_STACK (kitten->exports, eidx);
...
static unsigned export_literal (kitten *kitten, unsigned ilit) {
  ...
  assert (iidx < SIZE_STACK (kitten->exports));
  const unsigned eidx = PEEK_STACK (kitten->exports, iidx);
...
void kitten_clear (kitten *kitten) {
  ...
  while (!EMPTY_STACK (kitten->exports))
    kitten->import[POP_STACK (kitten->exports)] = 0;
...
#include "../util/runtime/attribute.h"
