...
#include "../../core/heuristic/heuristic.h"
...
static void dump_queue (kissat *solver) {
  const queue *const queue = &solver->heuristic->queue;
...
static void dump_scores (kissat *solver) {
  heap *heap = solver->heuristic->scores;
...
static void dump_export (kissat *solver) {
  const unsigned size = SIZE_STACK (solver->exports);
  for (unsigned idx = 0; idx < size; idx++)
    printf ("export[%u] = %u\n", LIT (idx),
            PEEK_STACK (solver->exports, idx));
}
...
void dump_map (kissat *solver) {
  ...
  for (unsigned idx = 0; idx < size; idx++) {
    ...
    const int elit = PEEK_STACK (solver->exports, idx);
