#include "resize.h"
#include "../runtime/allocate.h"
#include "../inline/inline.h"
#include "../misc/require.h"
...
void kissat_increase_size (kissat *solver, unsigned new_size) {
  ...
  NREALLOC_VARIABLE_INDEXED (links, heuristic->links);
  ...
  for(unsigned i =0;i<solver->heuristic->scores_list_size;++i){
      kissat_resize_heap (solver, solver->heuristic->scores_list[i], new_size);
  }
