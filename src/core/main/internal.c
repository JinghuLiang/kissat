...
kissat *kissat_init (void) {
...
#endif
  START (total);
  solver->heuristic = kissat_calloc(solver, 1, sizeof(heuristic_info));
  kissat_heuristic_init(solver);
  ...
  solver->stable_light_restart = GET_OPTION(lightrestart);
...
void kissat_release (kissat *solver) {
  ...
  kissat_heuristic_release(solver);
  kissat_dealloc(solver, solver->heuristic, 1, sizeof(heuristic_info));
  kissat_release_heap (solver, &solver->schedule);
  kissat_release_vectors (solver);
  kissat_release_phases (solver);

  RELEASE_STACK (solver->exports);
