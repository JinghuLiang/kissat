...
static inline double variable_score (kissat *solver, unsigned idx) {
  ...
  double relevancy;
  if (solver->stable){
    if(GET_OPTION(mab_enable)&&GET_OPTION(mab_vsids)){
      relevancy = kissat_get_heap_score (&solver->heuristic->vsids_info.scores, idx);
    }else{
      relevancy = kissat_get_heap_score (solver->heuristic->scores, idx);
    }
  }else
    relevancy = solver->heuristic->links[idx].stamp;
