  ...
  kissat *const solver = factoring->solver;
  {
    ...
    while (p != end) {
      ...
      if( GET_OPTION(mab_enable) && GET_OPTION(mab_vsids) ){
          kissat_update_heap (solver, &solver->heuristic->vsids_info.scores, idx, score);
      }else{
          kissat_update_heap (solver, solver->heuristic->scores, idx, score);
      }
    }
  }
  {
    const unsigned *p = end;
    links *links = solver->heuristic->links;
    ...
    solver->heuristic->queue.search.idx = queue->last;
    solver->heuristic->queue.search.stamp = queue->stamp;
  }
}
