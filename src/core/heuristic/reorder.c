...
static double *compute_weights (kissat *solver, bool norm) {
...
static void sort_active_variables_by_weight (kissat *solver,
  ...
  if (solver->stable) {
    ...
    struct links *links = solver->heuristic->links;
...
static void reorder_focused (kissat *solver) {
  ...
  double *weights = compute_weights (solver, false);
  unsigneds sorted;
...
static void reorder_stable (kissat *solver) {
  ...
  assert (solver->stable);
  bool norm = kissat_heuristic_rescale_scores (solver);
  double *weights = compute_weights (solver, norm);
  unsigneds sorted;
  sort_active_variables_by_weight (solver, &sorted, weights);
  heap *scores = &solver->heuristic->vsids_info.scores;
  unsigned* sorted_end = END_ARRAY(sorted);
  unsigned* sorted_begin = BEGIN_ARRAY(sorted);
  while (sorted_end != sorted_begin) {
      unsigned idx = *(--sorted_end);
      assert (ACTIVE (idx));
