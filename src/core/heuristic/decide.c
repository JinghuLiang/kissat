...
static unsigned last_enqueued_unassigned_variable (kissat *solver) {
  ...
  unsigned res = solver->heuristic->queue.search.idx;
  if (values[LIT (res)]) {
    do {
      res = links[res].prev;
      assert (!DISCONNECTED (res));
    } while (values[LIT (res)]);
    kissat_update_queue (solver, &solver->heuristic->queue, links, res);
  }
#ifdef LOGGING
...
static unsigned largest_score_unassigned_variable (kissat *solver) {
  ...
  const value *const values = solver->values;
  while (values[LIT (res)] || !ACTIVE (res)) {
    kissat_pop_max_heap (solver, scores);
    res = kissat_max_heap (scores);
  }
  if (solver->stable && GET_OPTION (mab_enable))
    solver->heuristic->mab_info.mab_decisions++;
