...
#include "../io/print.h"
#include "../../core/misc/report.h"
#include "../runtime/resources.h"
#include "../../core/cdcl/restart.h"
...
static void update_mode_limit (kissat *solver, uint64_t delta_ticks) {
  ...
  if (limits->mode.count & 1) {
    limits->mode.ticks = statistics->search_ticks + delta_ticks + solver->ticks_pool;
    solver->ticks_pool = 0;
#ifndef QUIET
    assert (solver->stable);
    kissat_phase (solver, "stable", GET (stable_modes),
                  ...
                  FORMAT_COUNT (delta_ticks + solver->ticks_pool));
#endif
  } else {
...
bool kissat_switching_search_mode (kissat *solver) {
  ...
  limits *limits = &solver->limits;
  statistics *statistics = &solver->statistics;
  static uint64_t base = 2e6;
  static bool clam_round = false;

  if (limits->mode.count & 1){
      if(CONFLICTS>=base){
          if(clam_round){
              base = CONFLICTS + 50 * 1024;
              clam_round = false;
          }else if(AVERAGE(faster_stable_dc) < AVERAGE(slow_stable_dc)*0.97 ){
              clam_round = true;
              //printf("c shrik C: %lu , SS: %lf SF: %lf\n", CONFLICTS, AVERAGE(slow_stable_dc), AVERAGE(fast_stable_dc) );
              //solver->ticks_pool = limits->mode.ticks - statistics->search_ticks;
              limits->mode.ticks = statistics->search_ticks;
              //}else if(!extended_stable && AVERAGE(fast_stable_dc) >= AVERAGE(slow_stable_dc)*1.02){
              //extended_stable = true;
              //printf("c shrik C: %lu , SS: %lf SF: %lf\n", CONFLICTS, AVERAGE(slow_stable_dc), AVERAGE(fast_stable_dc) );
              //limits->mode.ticks += solver->delta_ticks*0.2;
          }else{
              base += 1024;
          }
      }
