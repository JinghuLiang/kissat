...
int kissat_analyze (kissat *solver, clause *conflict) {
  ...
  do {
    ...
    if (!solver->probing && solver->stable && GET_OPTION (mab_enable))
      solver->heuristic->mab_info.mab_conflicts++;
    ...
    if (one_literal_on_conflict_level (solver, conflict, &conflict_level))
      ...
      INC (conflicts);
      if(solver->stable && !solver->probing){
          INC (stable_conflicts);
      }
      ...
      if (!solver->probing && solver->stable && solver->heuristic->stable_heuristic == CHB){
