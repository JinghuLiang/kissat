...
int kissat_search (kissat *solver) {
  ...
  if (!res && searching (solver)) {
    ...
    while (!res) {
      ...
      else if (kissat_light_restarting (solver)) {
        clause *conflict = kissat_light_restart (solver);
        if (conflict)
          res = kissat_analyze (solver, conflict);
      }
