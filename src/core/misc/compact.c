...
#include "../../util/data-structures/resize.h"
...
unsigned kissat_compact_literals (kissat *solver, unsigned *mfixed_ptr) {
  ...
  for (all_variables (iidx)) {
    ...
    if (flags->fixed) {
      ...
      const int elit = PEEK_STACK (solver->exports, iidx);
      if (elit) {
        ...
        POKE_STACK (solver->exports, iidx, 0);
      } else
...
static unsigned map_idx (kissat *solver, unsigned iidx) {
  int elit = PEEK_STACK (solver->exports, iidx);
  if (!elit)
    return INVALID_IDX;
  assert (elit);
  const unsigned eidx = ABS (elit);
...
static void compact_queue (kissat *solver) {
  ...
  links *links = solver->heuristic->links, *l;
  unsigned *p = &solver->heuristic->queue.first, prev = DISCONNECT;
  solver->heuristic->queue.stamp = 0;
  for (unsigned idx; !DISCONNECTED (idx = *p); p = &l->next) {
    const unsigned midx = map_idx (solver, idx);
    assert (midx != INVALID_IDX);
    l = links + idx;
    l->prev = prev;
    l->stamp = ++solver->heuristic->queue.stamp;
    if (idx == solver->heuristic->queue.search.idx) {
      solver->heuristic->queue.search.idx = midx;
      solver->heuristic->queue.search.stamp = l->stamp;
    }
    *p = prev = midx;
  }
  solver->heuristic->queue.last = prev;
  *p = DISCONNECT;
...
static void compact_export (kissat *solver, unsigned vars) {
  ...
  const size_t size = SIZE_STACK (solver->exports);
  assert (size <= UINT_MAX);
  assert (size == solver->vars);
  for (unsigned iidx = 0; iidx < size; iidx++) {
    const unsigned elit = PEEK_STACK (solver->exports, iidx);
    if (!elit)
      continue;
    const unsigned midx = map_idx (solver, iidx);
    if (midx == INVALID_IDX)
      continue;
    POKE_STACK (solver->exports, midx, elit);
  }
  RESIZE_STACK (solver->exports, vars);
  SHRINK_STACK (solver->exports);
#ifndef NDEBUG
  assert (SIZE_STACK (solver->exports) == vars);
...
void kissat_finalize_compacting (kissat *solver, unsigned vars,
  ...
  for(unsigned i =0;i<solver->heuristic->scores_list_size;++i){
      compact_scores (solver, solver->heuristic->scores_list[i], vars);
  }
