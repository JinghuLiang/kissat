...
static void flush_unit (kissat *solver) {
  clause *conflict = kissat_search_propagate (solver);
  assert (!conflict);
  assert (kissat_trail_flushed (solver));
}

static void add_unit_to_satisfy_redundant_clause (kissat *solver) {
...
static void add_unit_to_satisfy_irredundant_clause (kissat *solver) {
...
static void just_import_and_activate_four_variables (kissat *solver) {
  int ilit;
...
static void add_large_redundant_clause (kissat *solver) {
...
static void add_large_irredundant_clause (kissat *solver) {
  assert (solver->vars == 4);
  PUSH_STACK (solver->clause, 0);
...
static void test_collect (void) {
  ...
  for (unsigned i = 0; i < (1u << bits); i++) {
    ...
    if (i & (i << 9)) {
      ...
      bool flush = !(i & (i << 10));
      kissat_resume_sparse_mode (solver, flush, 0);
    } else if (i & (i << 10)) {
      if (!redundant_unit)
        add_unit_to_satisfy_redundant_clause (solver);
      if (!irredundant_unit)
...
void tissat_schedule_collect (void) { SCHEDULE_FUNCTION (test_collect); }

#else

void tissat_schedule_collect (void) {}
